...
class ProductFlat extends Model implements ProductFlatContract
{
    ...
    public function hasBeenOrderedByCustomer()
    {
        $customerOrderedProductSKUs = [];
        $orders = auth()->guard('customer')->user()->all_orders()->get();
        foreach($orders as $order) {
            foreach($order->items as $item) {
                if (!in_array($item->sku, $customerOrderedProductSKUs)) {
                    $customerOrderedProductSKUs[] = $item->sku;
                }
            }
        }

        if (in_array($this->sku, $customerOrderedProductSKUs)) {
            return true;
        }
        return false;
    }
