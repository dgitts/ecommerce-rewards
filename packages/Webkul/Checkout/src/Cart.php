...
//            $data['shipping_address'] = $cart->billing_address->toArray();
...
//        dd($shippingAddressData);
    ...
    /**
     * Returns hasEnoughPoints
     *
     */
    public function hasEnoughPoints()
    {
        $customerPoints = $this->getCurrentCustomer()->user()->reputation;
        $customerPointsAfterCheckout = $customerPoints - intval($this->getCart()->grand_total);
...
//            dd(Arr::except($data['shipping_address'], ['id', 'cart_id']));
...
//                'base_shipping_discount_amount' => $data['selected_shipping_rate']['base_discount_amount'],
    ...
    private function fillCustomerAttributes(): array
    {
        ...
        if ($user) {
            ...
            $attributes['customer_points'] = $user->reputation;
        }
